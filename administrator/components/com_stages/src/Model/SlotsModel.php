	...
	*/
	...
	public function getItems()
	{
		...
		foreach ($items as $oneItem)
		{
			...
			$$dt = new Date( $oneItem->slot_date );
            
			$oneItem->slot_date = $$dt->format('l').' '.$$dt->format('d').' '.$$dt->format('F').' '.$$dt->format('Y');

			$oneItem->slot_type = AgendaSlot::translateType($oneItem->slot_type);
			$oneItem->slot_periode = AgendaSlot::translateSchedule($oneItem->slot_periode);
                       
			$oneItem->slot_place = AgendaSlot::translatePlace($oneItem->slot_place);
			$oneItem->slot_public = AgendaSlotGroup::translateLabel($oneItem->slot_public). ' ' . 
                AgendaSlotGroup::translateSubLabel($oneItem->slot_public);;
