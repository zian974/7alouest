 ...
 * @since  0.1.0
 */
class StagiaireModel extends AdminModel
{
	/**
	 ...
	 * @since  0.1.0
	 */
	public $typeAlias = 'com_stages.stagiaire';
	...
	public function getForm($data = array(), $loadData = true)
	{
        ...
        $form->setFieldAttribute('slot_id', 'required', 'false');   
	...
	protected function loadFormData()
	{
		...
		if (empty($data))
		{
			...
			// Support for multiple or not foreign key field: horaire
			$array = array();

			foreach ((array) $data->horaire as $value)
			{
				if (!is_array($value))
				{
					$array[] = $value;
	...
	/**
	 * Method to duplicate an Stagiaire
	 *
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = Factory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__stages_stagiaires');
				$max             = $db->loadResult();
